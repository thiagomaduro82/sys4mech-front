...
export const CustomerList: React.FC = () => {
    ...
    const canDeleteCustomers = useHasPermission('CUSTOMER_DELETE');

    const navigate = useNavigate();
    const [rows, setRows] = useState<ICustomerDetail[]>([]);
        ...
        CustomerService.getAll(
            ...
            .then((response) => {
                ...
                if (response instanceof Error) {
                    handleOpenModal('error', 'Error fetching customers', response.message, null);
                } else {
    ...
    const handleDelete = () => {
        if (selectedUuid) {
            CustomerService.deleteByUuid(selectedUuid)
                .then((response) => {
                    if (response instanceof Error) {
                        handleOpenModal('error', 'Error deleting customer', response.message, null);
                    } else {
                        handleOpenModal('success', 'Customer deleted successfully', '', null);
        ...
        <BasicLayout
            ...
            <Box component={Paper} sx={{ p: 1, m: 1, width: 'auto' }}>
                {canWriteCustomers && <Button
                    ...
                    onClick={() => navigate('/customers/detail/create')}
                >
                    New
                </Button>}
                <TableContainer>
                            ...
                            {rows.map(row => (
                                <TableRow key={row.uuid}>
                                    ...
                                    <TableCell sx={{ textAlign: "center" }}>
                                        {canWriteCustomers && <IconButton size="small" color="info" sx={{ marginRight: 1 }} title="Edit record"
                                            onClick={() => navigate(`/customers/detail/${row.uuid}`)}>
                                            <EditNoteIcon fontSize="inherit" />
                                        </IconButton>}

                                        {canDeleteCustomers && <IconButton size="small" color="error" title="Delete record"
                                            onClick={() => handleOpenModal("confirmation", "Delete customer", "Are you sure you want to delete this customer? This operation cannot be undone!", row.uuid)}>
